...
void ADXL345_get_Gxyz(float *xyz){
	int i;
	int xyz_int[3];
	readAccel(xyz_int);
	for(i=0; i<3; i++){
...
void ADXL345_setAxisGains(float *_gains){
	int i;
	for(i = 0; i < 3; i++){
		gains[i] = _gains[i];
	}
}
void ADXL345_getAxisGains(float *_gains){
	int i;
	for(i = 0; i < 3; i++){
		_gains[i] = gains[i];
	}
...
void ADXL345_setDoubleTapLatency(int floatTapLatency) {
...
void ADXL345_setDoubleTapWindow(int floatTapWindow) {
...
void ADXL345_setRate(float rate){
	byte _b,_s;
	int v = (int) (rate / 6.25);
	int r = 0;
	while (v >>= 1)
