...
unsigned char picture1[1280*800*3]={0};

void store_img(u32 x, u32 y, u32 disp_base_addr, unsigned char * addr, u32 size_x, u32 size_y);
...
int main(void)
{
	...
	int Status_ov5640;
				 ...
				 */
		  init_platform();
				Status_ov5640 = XGpio_Initialize(&IIC, XPAR_AXI_GPIO_0_DEVICE_ID);
				if (Status_ov5640 != XST_SUCCESS) {
					return XST_FAILURE;
				}
				 delayms(200);
			     SIO_D_OUT;
	...
	xil_printf("Starting storing the image \n\r");

	                         delayms(200);

			    	         SD_Init();
			    	         //SD_Transfer_read("test.bin",(u32)picture1,640*480*3+1);
			    	         //show_img(0,0,srcBuffer,&picture1[0],640,480);
			    	         store_img(0,0,0x10000000,&picture1[0],1280,800);
			    	         SD_Transfer_write("test_p.txt",(u32)picture1,1280*800*3+1);
			    	         xil_printf("Finishing storing the image \n\r");
            cleanup_platform();
	return 0;
}
...
void store_img(u32 x, u32 y, u32 disp_base_addr, unsigned char * addr, u32 size_x, u32 size_y)
{
    ...
    start_addr = disp_base_addr + 3*x + y*3*2048;
    for(j=0;j<size_y;j++)
    {
        for(i=0;i<size_x;i++)
        {
        	temp = Xil_In32(start_addr+(i+j*2048)*3);
        	b = (temp)&(0xFF);
            g = (temp>>8)&(0xFF);
            r = (temp>>16)&(0xFF);
            *(addr+(i+j*size_x)*3+0)=(u8)b;
            *(addr+(i+j*size_x)*3+1)=(u8)g;
            *(addr+(i+j*size_x)*3+2)=(u8)r;
        }
    }
    Xil_DCacheFlush();
}
