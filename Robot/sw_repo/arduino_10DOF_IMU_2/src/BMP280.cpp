...
*/
/**************************************************************************/
void DFRobot_BMP280::write8(uint8_t reg, uint8_t value)
{
	uint8_t temp[2];
	temp[0] = reg;
	temp[1] = value;
	i2c_write(BMP280_device, _i2caddr, temp, 2);
}
...
*/
/**************************************************************************/
uint8_t DFRobot_BMP280::read8(uint8_t reg)
{
	uint8_t value, status;
	status = i2c_write(BMP280_device, _i2caddr, &reg, 1);
	status = i2c_read(BMP280_device, _i2caddr, &value, 1);
	return value;
}
...
*/
/**************************************************************************/
uint16_t DFRobot_BMP280::read16(uint8_t reg)
{
	uint8_t temp[2] = {0};
	uint16_t value=0;
	i2c_write(BMP280_device, _i2caddr, &reg, 1);
	i2c_read(BMP280_device, _i2caddr, temp, 2);
	value = (temp[0] << 8) | temp[1];
	return value;
}

uint16_t DFRobot_BMP280::read16_LE(uint8_t reg) {
...
*/
/**************************************************************************/
int16_t DFRobot_BMP280::readS16(uint8_t reg)
{
	return (int16_t)read16(reg);
}

int16_t DFRobot_BMP280::readS16_LE(uint8_t reg)
{
...
uint32_t DFRobot_BMP280::read24(uint8_t reg)
{
	uint8_t temp[3] = {0};
	uint32_t value=0;
	i2c_write(BMP280_device, _i2caddr, &reg, 1);
	i2c_read(BMP280_device, _i2caddr, temp, 3);
	value = (temp[0] << 16) | (temp[1] << 8) | temp[2];
	return value;
}
